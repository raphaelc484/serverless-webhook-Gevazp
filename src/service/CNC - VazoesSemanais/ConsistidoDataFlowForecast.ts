...
export async function ConsistidoDataFlowForecast({
  docType,
  sudeste,
  sul,
  nordeste,
  ...
  const findDateFile = await prisma.tbl_flow_forecast.findMany({
    where: {
      dat_forecast: findLastDateDocType.dat_file_publi,
    },
  });

  if (findDateFile.length === 0) {
    ...
    await prisma.tbl_flow_forecast.update({
      where: {
        num_forecast: findDateFile[0].num_forecast,
      },
      data: {
    ...
    await prisma.tbl_flow_forecast.update({
      where: {
        num_forecast: findDateFile[1].num_forecast,
      },
      data: {
    ...
    await prisma.tbl_flow_forecast.update({
      where: {
        num_forecast: findDateFile[2].num_forecast,
      },
      data: {
    ...
    await prisma.tbl_flow_forecast.update({
      where: {
        num_forecast: findDateFile[3].num_forecast,
      },
      data: {
