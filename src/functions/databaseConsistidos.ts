...
import { ConsistidoDataFlowForecast } from "../service/CNC - VazoesSemanais/ConsistidoDataFlowForecast";
...
export const handler: S3Handler = async (event) => {
  ...
  if (dataRead) {
    try {
      ...
      if (listFilesBucket[0].includes("REV")) {
        ...
        await ConsistidoDataFlowForecast({
          docType,
          sudeste,
          sul,
          nordeste,
  ...
  await MicrosoftTeam({
    title: "Previsão de Vazões Atualizada - Consistido",
    message:
